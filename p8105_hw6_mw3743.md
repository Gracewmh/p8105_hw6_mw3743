...
<img src="p8105_hw6_mw3743_files/figure-gfm/unnamed-chunk-13-1.png" width="90%" />

The comparison of RMSE values shows clear differences between the three
models. Model fit0, which is the model I fitted has the highest RMSE.
This means it is too simple and cannot capture important patterns in the
data. Model fit1, which includes all interactions between head
circumference, length, and sex, has a lower RMSE but shows more
variability, meaning it is too complex and overfits the data. Model
fit2, which balances complexity and simplicity, has the lowest RMSE and
the least variability. This shows that fit2 is the best model for
